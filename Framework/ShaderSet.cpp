...
void ShaderSet::init(ID3D11Device* device, const ShaderSetDesc& desc, const InputLayoutDesc & layout, bool layoutEnabled)
{
	...
	if (blobs[ShaderStage::kCompute])
	{
		...
		if (FAILED(hr))
		{
			panicF("Failed to create compute shader");
		}
	}
